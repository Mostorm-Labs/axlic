...
Json command_result(
    const bool ok, std::string_view code, std::string_view category, std::optional<Json> data = std::nullopt) {
  Json result = {{"contract_version", "1.0"},
                 {"ok", ok},
                 {"code", code},
                 {"category", category},
                 {"retryable", false},
                 {"correlation_ref", "a0-test-fixture"}};
  if (data) {
...
int main(const int argc, const char* const argv[]) {
  try {
    ...
    return command == "status" ? status_command(runtime) : entitlement_command(runtime, argv[2]);
  } catch (...) {
